...
1. **Site aberto no navegador** — URL hospedada (Render):
...
2. **APK Android installável** — baixe em *Releases* (`nightly`) ou em *Actions → Build APK Android → artifact*:
...
3. **(Opcional) ESP32 real** — o firmware envia para o mesmo backend hospedado:
   - `firmware/esp32/include/secrets.h` com `API_URL = https://SEU-SITE.onrender.com/api/readings`
...
| ---- | ---- | ---------- |
| Site ao vivo | URL do Render (`https://...onrender.com`) | Abrir ~30 s antes; o plano gratuito "dorme" em ~15 min ocioso |
| APK Android | GitHub → Releases (`nightly`) | Instalação direta no celular |
| API docs | `https://...onrender.com/docs` | Swagger interativo (mostra as rotas do ESP32) |
