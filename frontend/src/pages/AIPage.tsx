  ...
  if (!status.model_loaded) {
    return (
      <div className="space-y-4">
        <h2 className="bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-2xl font-extrabold tracking-tight text-transparent dark:from-white dark:to-slate-300">IA / Modelo</h2>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={refresh}
          className="rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold text-white shadow-md transition-all hover:bg-slate-800 hover:shadow-lg active:scale-[0.98] dark:bg-white dark:text-slate-900"
        >
          ↻ Atualizar
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <dl className="space-y-1.5 text-sm">
            <div className="flex justify-between gap-2">
              ...
              <dd className="font-mono text-xs text-slate-700 dark:text-slate-300">{status.dataset ?? '—'}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-slate-500 dark:text-slate-400">Origem</dt>
              <dd className="text-xs text-slate-700 dark:text-slate-300">{status.dataset_origin ?? '—'}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-slate-500 dark:text-slate-400">Variáveis (features)</dt>
              <dd className="text-xs text-slate-700 dark:text-slate-300">{(status.features ?? []).map(featureLabel).join(', ')}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-slate-500 dark:text-slate-400">Rótulos</dt>
              <dd className="text-xs text-slate-700 dark:text-slate-300">{status.labels ? status.labels.join(', ') : '—'}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          {cm ? (
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">Métricas não disponíveis para este modelo.</p>
          )}
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {status.overfitting.warning && (
              <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">ALERTA DE SOBREAJUSTE</span>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {cm && (
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h3 className="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Matriz de confusão</h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={featureImportance} layout="vertical" margin={{ top: 5, right: 20, bottom: 0, left: 20 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [Number(value).toFixed(4), 'Importância']}
                  />
                  <Bar dataKey="importance" fill={isDark ? '#38bdf8' : '#0284c7'} radius={[0, 8, 8, 0]} isAnimationActive={true} animationDuration={700} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800 dark:border-amber-800/30 dark:bg-amber-950/30 dark:text-amber-300">
        ...
        <p className="mt-2 text-[11px] text-amber-700 dark:text-amber-400/80">
          Glossário: <strong>Recall</strong> = achar os verdadeiros · <strong>F1</strong> = equilíbrio precisão/recall ·{' '}
          <strong>MAE/RMSE</strong> = erros médios · <strong>overfitting</strong> = decora treino · <strong>acurácia</strong> = % acertos.
        </p>
      </div>
    </div>
  );
